...
/*   Updated: 2022/08/16 20:54:48 by dritsema      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
t_point3d	orthographic_transform(t_point3d p, t_fdf *fdf)
{
	...
	new_p.x *= (fdf->zoom / fdf->faspect_ratio);
	new_p.y *= (fdf->zoom / fdf->faspect_ratio);
	new_p.z += fdf->z_offset;
...
{
	fdf->orth.m[0][0] = fdf->scale;
	fdf->orth.m[1][1] = -fdf->scale;
	fdf->orth.m[2][2] = 1;
	fdf->orth.m[3][3] = 1;
}
...
void	orthographic_init(t_fdf *fdf)
{
	...
	fdf->x_offset = 0;
	fdf->y_offset = 0;
	fdf->z_offset = fdf->map_width + fdf->map_height;
	fdf->ffar = 1000;
	fdf->zoom = 5000 / (fdf->map_height);
	fdf->ffov = 60;
